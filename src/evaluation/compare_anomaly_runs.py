...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare anomaly detection runs with statistical tests")
    parser.add_argument("--records-path", default=str(DEFAULT_RECORDS_PATH))
    parser.add_argument("--metric", default="test_pr_auc", help="Metric column to compare")
    parser.add_argument("--group-field", default="feature_profile", help="Field to create A/B groups")
    parser.add_argument("--group-a", required=True, help="Group A value")
    parser.add_argument("--group-b", required=True, help="Group B value")
    parser.add_argument("--model", default=None, help="Filter to a specific model (e.g. isolation_forest)")
    parser.add_argument("--split-path", default=None, help="Filter to a specific split_path (e.g. path_a)")
    parser.add_argument("--max-runs-per-group", type=int, default=None)
    ...
    if args.model:
        df = df[df["model"].astype(str) == args.model]
    if args.split_path:
        df = df[df["split_path"].astype(str) == args.split_path]
    ...
    if group_a.empty or group_b.empty:
        raise ValueError(
            f"Not enough data. Group A rows={len(group_a)}, Group B rows={len(group_b)}"
        )
    ...
    if args.model:
        result["model_filter"] = args.model
    if args.split_path:
        result["split_path_filter"] = args.split_path
