...
def _build_extra_trees_params(base: dict, seed: int, n_jobs: int) -> dict:
    params = dict(base)
    params.update(
        {
...
def run_extra_trees(config: dict | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Train Task B classification model (ExtraTrees + Optuna + MLflow)"
    )
    ...
    if active_model != "extra_trees":
        logger.warning(
            "Active classification model is '{}' but ExtraTrees entrypoint was invoked directly.",
            active_model,
        )

    model_spaces = runtime_cfg.get("model_spaces", {})
    search_space = model_spaces.get("extra_trees")
    if not search_space:
        raise KeyError(
            "Missing classification.ml.models.extra_trees search space in model_config.yaml"
        )
    ...
    run_name = f"classification_extra_trees_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        if args.no_optuna:
            ...
            def objective(trial) -> float:
                ...
                model_params = _build_extra_trees_params(
                    trial_params,
                    seed=hpo_seed,
                    n_jobs=int(threading_plan["threads_per_trial"]),
                )
        ...
        logger.success(
            "ExtraTrees complete | accuracy={:.4f} f1_macro={:.4f} f1_weighted={:.4f} pr_auc_weighted={:.4f}",
            accuracy,
            f1_macro,
            f1_weighted,
            pr_auc_weighted,
