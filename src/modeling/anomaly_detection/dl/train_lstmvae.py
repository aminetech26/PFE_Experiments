"""
Training script for the LSTM Variational Autoencoder model on Costa preprocessed data.
...
from src.modeling.anomaly_detection.dl.lstm_vae_model import build_lstm_vae_model
...
DEFAULT_CHECKPOINT_DIR = PROJECT_ROOT / "experiments" / "checkpoints" / "lstmvae"
...
def main():
    ...
    if not parquet_path.exists():
        ...
        logger.warning("Please run: python -m src.data.preprocess_lstmvae")
        return
    ...
    for i, (units, latent, lr, lookback, batch) in enumerate(grid):
        ...
        if score_val > current_best_val or best_score == float('inf'):
            ...
            best_model_path = checkpoint_dir / "lstmvae_best.keras"
    ...
    metrics_path = metrics_dir / "lstmvae_results.json"
